StackCalc32: Apple Watch RPN CalculatorVersion: 1.0 (Architecture & UI Specification)Target Platform: watchOS 10+ (Apple Watch Series 7, 8, 9, Ultra)Frameworks: SwiftUI, Charts, Swift 5.9, C/C++Core Engine: Advanced (WP43S) or Free42 C/C++ Core1. Executive SummaryStackCalc32 brings the mathematical rigor of the HP-32SII and Advanced to the Apple Watch. It solves the "micro-button" problem of watch calculators by utilizing a Continuous Vertical Ribbon (a scrolling UI), Screen-Wide Gestures (eliminating modifier keys), and Native Apple Input (for variables and equations). The aesthetic mimics the molded plastic of the Pioneer chassis (e.g., SwissMicros Modern).2. System ArchitectureThe application strictly decouples the UI from the math engine using a Local Swift Package Manager (SPM) architecture.2.1 Workspace StructureWatchCalcWorkspace/
├── StackCalc32_AppTarget/
