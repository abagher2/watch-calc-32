...
# Find and delete the old CR2032 bucket
to_delete = []
for dwg in board.GetDrawings():
    ...
    if layer_name == "User.Comments" or layer_name == "F.Silkscreen":
        if isinstance(dwg, pcbnew.PCB_TEXT):
            ...
            # If it's a rectangle in the top area
            if dwg.GetShape() == pcbnew.SHAPE_T_RECT:
                if dwg.GetStart().y / 1e6 < -110:
                    to_delete.append(dwg)
            elif dwg.GetShape() == pcbnew.SHAPE_T_SEGMENT:
                # Or lines in the top area
                if dwg.GetStart().y / 1e6 < -120:
                    to_delete.append(dwg)

for d in to_delete:
    board.Remove(d)

# Draw the exact physical battery position
batt_x = 57.0 * 1e6
batt_y = -128.1 * 1e6
radius = 10.5 * 1e6

# Draw Circle on User.Comments and F.Silkscreen
...
print("Updated KiCad board with accurate CR2032 footprint.")
